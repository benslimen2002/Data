...
class PedVLMT5(nn.Module):
    def __init__(self, config):
        ...
        # Simple parameter counting without peft
        total_params = sum(p.numel() for p in self.model.parameters())
        trainable_params = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        
        print('Trainable Parameters for LM model:')
        print(f'Trainable params: {trainable_params:,} || all params: {total_params:,} || trainable%: {100.0 * trainable_params / total_params:.1f}')
    ...
    def forward(self, text_enc, imgs, labels=None, bodypose=None, local_content_features=None, local_motion_features=None):
        ...
        if self.use_local_features and local_content_features is not None and local_motion_features is not None:
            ...
            if merged_embedding.shape[1] > 49:  # If we have text + image embeddings
                ...
                print(f"DEBUG: local_features shape before squeeze: {local_features.shape}")
                print(f"DEBUG: merged_embedding shape: {merged_embedding.shape}")
                
                # Remove all extra dimensions until we get [batch_size, hidden_size]
                while len(local_features.shape) > 2:
                    local_features = local_features.squeeze(1)
                
                print(f"DEBUG: local_features shape after squeeze: {local_features.shape}")
